  ...
  return (
    <section id="about" className="relative overflow-hidden bg-grid bg-navy">
        ...
        <div className="relative">
          <div className="relative" style={{ maxWidth: 480 }}>
            ...
            {/* Floating stat card — partially overlaps bottom-left corner of photo.
                Text colors use --on-accent which inject-theme.mjs auto-picks
                (dark for gold/yellow brands, white for blue/navy/red brands) so
                the chip passes WCAG AA contrast for any brand accent. Solid
                backgroundColor matches --accent so contrast auditors detect a
                real bg; the gradient still paints over it. */}
            <div
              style={{
                position: 'absolute',
                bottom: 54,
                ...
                backgroundImage: 'linear-gradient(135deg, rgb(var(--accent-light)) 0%, rgb(var(--accent)) 40%, rgb(var(--accent-dark)) 65%, rgb(var(--accent-light)) 100%)',
                padding: '16px 22px',
            ...
            >
              <div className="font-heading font-bold leading-none" style={{ fontSize: 40, color: 'rgb(var(--on-accent))' }}>{brandDNA.team.founder.yearsExp}</div>
              <div className="font-body text-[10px] uppercase tracking-wider leading-tight mt-1.5" style={{ color: 'rgb(var(--on-accent))', opacity: 0.92 }}>
      ...
      <div className="relative max-w-6xl mx-auto px-8 pb-16 grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {/* Vision card. Icon uses `text-gold` + `currentColor` so the
            dark-mode [data-theme-mode="dark"] .text-gold override (color-mix
            of --accent + white) auto-brightens it to pass WCAG SC 1.4.11
            (3:1) against the dark card surface. */}
        <div
          className="vision-mission-card overflow-hidden p-6"
        >
          <div className="flex items-center gap-3 mb-4">
            <div
              className="w-9 h-9 flex items-center justify-center flex-shrink-0 text-gold"
              style={{ background: 'rgb(var(--accent) / 0.18)', border: '1px solid rgb(var(--accent) / 0.55)' }}
            >
        ...
        {/* Mission card — same icon-contrast pattern as Vision. */}
        <div
          className="vision-mission-card overflow-hidden p-6"
        >
          <div className="flex items-center gap-3 mb-4">

...
// Brand-only palette: rating stars use the logo's medium blue (no yellow).
const Stars = () => (
  <div className="flex gap-0.5 text-[rgb(var(--accent))]">
    {[1,2,3,4,5].map(i => <span key={i} className="text-sm">&#9733;</span>)}
  </div>
);

// Third-party marks are rendered monochrome in the brand's deep blue so the
// palette stays 100% logo-derived (no Google red/yellow/green, no Facebook blue).
const GoogleIcon = () => (
  <svg className="w-5 h-5 flex-shrink-0 text-[rgb(var(--primary))]" viewBox="0 0 24 24" fill="currentColor">
...
function ReviewCard({ review }) {
  ...
  // When the brand has no public reviews yet, cards are illustrative samples:
  // drop the platform icon and label them plainly so nothing reads as a real
  // Google/Facebook review.
  const sample = Boolean(brandDNA.reviews?.show_zero_pills);
  return (
    <div className="card-elevated-dark border border-steel/25 h-full flex flex-col bg-navy-slate">
      <div className="flex items-center justify-between mb-3 p-6 pb-0">
        <div className="flex items-center gap-2">
          {!sample && (isFacebook ? <FacebookIcon /> : <GoogleIcon />)}
          <Stars />
      ...
      <div className="flex items-center gap-3 pt-4 border-t border-steel/25 px-6 pb-6">
        ...
        <div>
          ...
          <div className="font-body text-xs text-steel">{sample ? 'Sample review' : (isFacebook ? 'Facebook Review' : 'Google Review')}</div>
        </div>
      </div>
    </div>
  );
...
export default function Reviews() {
  ...
  return (
    <section id="reviews" className="relative overflow-hidden bg-navy">
      ...
      <div className="relative max-w-3xl mx-auto px-8 pt-12 text-center">
        ...
        <h2 className="section-h2 text-white uppercase mb-5">
      ...
      {brandDNA.reviews?.show_zero_pills && (
        <div className="relative max-w-7xl mx-auto px-8 mt-6 flex items-center justify-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 px-4 py-2 bg-navy-slate" style={{ border: '1px solid rgba(100,116,139,0.4)' }}>
            <GoogleIcon />
            <span className="font-body font-bold text-xs text-white uppercase tracking-wider">Google</span>
            <span className="font-body text-xs text-steel">0 reviews</span>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-navy-slate" style={{ border: '1px solid rgba(100,116,139,0.4)' }}>
            <FacebookIcon />
            <span className="font-body font-bold text-xs text-white uppercase tracking-wider">Facebook</span>
            <span className="font-body text-xs text-steel">0 reviews</span>
          </div>
        </div>
      )}
      ...
      <div className="relative max-w-7xl mx-auto px-8 mt-5">
        <div className="flex items-center gap-4">
          ...
          {/* Cards — centered so a short set (e.g. 2 reviews) sits in the middle
              instead of hugging the left with an empty third column. */}
          <div className="flex-1 flex flex-col lg:flex-row lg:justify-center items-stretch gap-5">
            {isDesktop
              ? reviews.slice(current, current + 3).map((review, i) => (
                  <div key={current + i} className="w-full lg:w-[360px] flex"><ReviewCard review={review} /></div>
                ))

...
export function HeroConnectedForm() {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[var(--color-surface-dark)] pt-24 pb-12 md:pt-28 md:pb-20">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_35%_15%,color-mix(in srgb, var(--color-accent) 22%, transparent),transparent_30%),linear-gradient(120deg,var(--color-ink)_0%,var(--color-primary-dark)_52%,var(--color-primary)_100%)]" />

      <div className="relative z-10 mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
        <div className="grid min-h-[720px] items-center gap-10 lg:grid-cols-[1fr_1.1fr]">
          <div className="relative z-20">
            ...
            <h1 data-scale="display" className="mt-5 max-w-[820px] font-black tracking-tight text-white [&_.text-accent]:text-accent-light">
          ...
          <div className="relative min-h-[680px]">
            {owner && (
              <div className="absolute bottom-0 left-0 hidden h-[640px] w-[440px] overflow-hidden rounded-t-[220px] bg-accent/15 ring-1 ring-accent/20 lg:block">
                {ownerImage ? (
                  <Image
                    src={ownerImage}
                    alt={owner.name}
                  ...
                  <div className="grid h-full w-full place-items-center text-8xl font-black text-white">
                    {owner.name.charAt(0)}
                  </div>
                )}
              </div>
            ...
            {owner && (
              <div className="absolute bottom-10 left-8 hidden w-[350px] rounded-2xl border border-white/15 bg-black/65 p-5 text-white shadow-2xl backdrop-blur-md lg:block">
                {showRating && (
            ...
            <div className="relative z-10 mx-auto max-w-[460px] lg:absolute lg:right-0 lg:top-1/2 lg:-translate-y-1/2">
              <div className="absolute -inset-4 rounded-[2rem] bg-accent/25 blur-3xl" />
              <div className="relative rounded-[2rem] border border-white/15 bg-white/10 p-4 shadow-2xl backdrop-blur-md">
                <ContactForm />

                {owner && (
                  <div className="mt-4 flex items-center gap-3 rounded-2xl bg-white/10 p-4 text-white">
            ...
            <div className="absolute bottom-0 right-0 hidden h-[540px] w-[560px] rounded-[3rem] border border-white/10 bg-white/[0.03] lg:block" />
          </div>
        </div>
      </div>
    </section>
